...
struct MAP_API DateVersion {
    ...
    int minor;       // 次版本号
    int revision;    // 修正版本号
    bool operator ==(const DateVersion& op) const;
};
...
/**
     ...
     * @brief UTM XYZ coordinate of a certain position
     */
struct MAP_API XYZWithHDG
{
    int hdg_valid;    // 航向角是否有效，0-无效，其他-有效
    double hdg;       // 航向角
    XYZ xyz;          // xyz坐标
    XYZWithHDG();
    XYZWithHDG(int hdg_validd, double hdgg, XYZ xyzz);
};

/**
     * @struct RoutingConstraint
     * @brief Constraint when routing
     */
struct MAP_API RoutingConstraint
{
    double max_curvature;    // 道路最大曲率限制
    double min_width;        // 道路最小宽度限制
};
...
public: // TODO:
    ...
    void get_version(DateVersion& map, DateVersion& map_lib) const;

    void get_eswn(double& north, double& south, double& east, double& west) const;

    // 加载地图灰度图，与opendrive格式地图加载与否无关
    bool load_binary_map(const char *file_path);

    // 基于灰度地图判断点是否在地图可行驶区域内，单次耗时为微秒级
    bool is_point_on_map(const XYZ& xyz) const;
    ...
    ErrorCode plan_route(const XYZWithHDG& start_point_with_hdg, const std::vector<XYZ> &way_points, const XYZ& end_point, Route& route) const;
    ...
    ErrorCode find_slzs_global(const XYZ& xyz, std::vector<SLZ>& slzs)const;
	...
	bool is_inside_junction(const JunctionId& junction_id, const XYZ& pos) const;    // junction_id输入为junction id或者road id都可以
    ...
    bool get_bus_stop_by_xyz(const XYZ& xyz, BusStop& bus_stop) const;

    void get_bus_stop_corner(const BusStopSpare& bus_stop, std::vector<XYZ>& corner) const;
    ...
    ErrorCode get_rotation_by_slz(const SLZ& slz, Rotation& rotation) const;                                 // 耗时最少
    ErrorCode get_rotation_by_xyz(const XYZ& point, Rotation& rotation) const;                               // 耗时最多
    ErrorCode get_rotation_by_xyz_with_hint(const LaneId& hint, const XYZ& point, Rotation& rotation) const; // hint是正确的，耗时与最少的差不多；hint是错的，耗时与最多的差不多

    ErrorCode get_z_by_sl(SLZ& slz) const;                                     // 耗时最少

    ErrorCode get_z_by_xy_with_road_id(const RoadId& road_id, XYZ& xyz) const; // hint是正确的，耗时与最少的差不多；hint是错的，耗时与最多的差不多

    ErrorCode get_z_by_xy_with_last_z(double last_z, XYZ& xyz) const;          // 耗时最多
